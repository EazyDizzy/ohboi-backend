...
pub async fn parse() -> Result<String, reqwest::Error> {
    let mut all_products: Vec<ParsedProduct> = vec![];

    for page in 0..1000 {
        let data = parse::requester::get_data(get_next_page_url(page).as_str()).await?;
        println!("page {}", page + 1);
        let document = Html::parse_document(&data);
        let current_length = all_products.len();

        extract_products(document, &mut all_products);
...
fn get_next_page_url(current_page: i32) -> String {
    "https://mi-shop.com/ru/catalog/smartphones/page/{page}/"
        .replace("{page}", (current_page + 1).to_string().as_ref())
}

fn extract_products(document: Html, all_products: &mut Vec<ParsedProduct>) {
